...
(Diagram generated below —  exported SVG into GitHub as `/assets/architecture.svg` )
...
7.3 Circuit Diagram
(Generated below —  exported SVG as `/assets/circuit.svg`)
7.4 Power Plan
Question	Response
...
Output behavior:
`draw_bat()` renders each paddle as a rotated lollipop bat with rubber face sectors, wooden handle, grip wraps, and knob — rotated by a swing offset applied on each hit and decayed by factor 0.86 per frame. `draw_trail()` renders a fading yellow ghost trail. Ball color interpolates from white (slow) toward amber-red (fast) relative to `MAX_BALL_SPEED`. Scores render in large gray text; rally counter shows in orange when hits ≥ 4.
Communication logic:
No network communication. All communication is local GPIO reads within the same board.
Reset behavior:
`reset_ball()` re-centers the ball at screen center and restores base velocity toward the player who just lost the point. `reset_paddles()` restores paddle heights, vertical positions, and zeroes velocity and swing. ESC during play returns to Menu without resetting scores; starting a new game from Menu resets both scores to zero.
8.3 Code Flowchart
(Generated below —  exported SVG as `/assets/flowchart.svg`)
---
9. Bill of Materials
9.1 Full BOM
...
3.5 mm speaker / headphones	1	Yes	No	0	Any	Audio output for synthesized sounds
...
Tester	What They Did	What Confused Them	What They Enjoyed	What Was Changed
aadit	Played 3 full rounds on Medium	Wasn't sure which sensor was theirs at first	Speed increase during rally — "it gets scary fast"	Added color tint to each paddle (blue / green) to distinguish sides
rushikesh	Played solo testing both sides	Hard mode felt too punishing on first try	The bat swing animation on hit	Added Easy mode with larger paddle and slower ball
suyog	Stress-tested rapid tapping	No confusion	Rally counter — "loved seeing RALLY x7"	Lowered rally counter threshold from 5 to 4 hits
unnati	First time playing	Did not notice the difficulty selector on first launch	Physical sensor feel vs keyboard — "sensor is way more satisfying"	Added blinking "PRESS ENTER to Start" prompt with animated alpha
---
15. Build Documentation
15.1 Fabrication Process
No structural fabrication was required. The project is entirely electronics and software on an existing Raspberry Pi board with off-the-shelf sensor modules. The build process was:
...
(Upload images to `/assets/` folder and embed below)
```md
...
Coding: Rust's ownership model initially slowed development (borrowing `rl` for drawing while also reading GPIO), but the borrow checker caught bugs at compile time that would have been runtime headaches in Python. Audio synthesis from scratch (generating WAV buffers via sine math) was unexpectedly straightforward and taught the fundamentals of PCM audio.
Integration: The biggest integration challenge was getting Raylib's audio system initialized before any sound play calls — ordering matters and the `expect()` guard was the right call.
Fabrication: No fabrication was needed, which freed significant time for polish. The lesson: when the project is software-first, invest that saved time in code quality and playtesting, not structure.
18.3 Design Reflection
Designing for delight: The swing animation on paddle hit was added late but had the biggest impact on player satisfaction — it makes hits feel physical even though everything is on screen.
Clarity: The difficulty stats panel (showing actual gravity and ball speed numbers) let players make informed choices and set expectations before a round — much better than vague "Easy / Hard" labels alone.
Physical interaction: The capacitive sensor is significantly more satisfying than a keyboard key. The act of touching a surface to flap creates a direct physical connection to the game that a keypress doesn't. This reinforced the value of even minimal physical hardware in a game experience.
Iteration: The rally counter threshold was tuned through playtesting (5 → 4). Small numbers matter — showing "RALLY x4" more frequently made players feel successful sooner and kept energy high during slower rounds.
18.4 If We Had One More Hour
We would add a win condition: first player to 7 points wins, with a brief fullscreen celebration animation and automatic return to the menu. This would make each match feel conclusive rather than indefinitely continuing, and give players a reason to start again. Second priority would be a startup "tap your sensor" calibration prompt so new players immediately understand the hardware interface.
